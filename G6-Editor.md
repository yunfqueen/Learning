...
* 1.<font coloe="#f00" face="黑体"> G6Editor与Extjs的结合</font>
* 2.<font coloe="#f00" face="黑体">自动绘制</font> 
  * 在其他页面中添加节点或修改内容后可根据数据在页面中自动绘制出新的流程图
* 3.<font coloe="#f00" face="黑体">工作流模版</font> 
  * 实例可能会和提供模版有差别，比如报销单，模版中的某个流程点为审核人员，实例中的该点有可能会在审核完成后变为审核人的头像并且后面会有个小对号表示审核已完成
* 4.node节点中自定义属性的动态修改
* 5.node图形的修改（如图形右半部分的icon动态修改成我们需要的图片）
* 6.page中canvas的属性研究
* 7.edge控制节点连接线个数
* 8.缩略图需求(后期需考虑该需求)
* 9.edge上的文字，是否可换为图标icon，其颜色和倾斜角度是否可变化
